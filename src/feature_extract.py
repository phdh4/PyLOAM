...
class FeatureExtract:
    ...
    def compute_curvatures(self, cloud):
        ...
        scan_start_id = []
        scan_end_id = []
        for val in range(0, self.used_line_num):
            indices = np.where(cloud[:, self.RING_INDEX] == val)[0]
            if indices.size > 0:
                scan_start_id.append(indices[0] + 5)
                scan_end_id.append(indices[-1] - 5)
            else:
                print(f"No data for ring {val}, skipping.")
                scan_start_id.append(None)
                scan_end_id.append(None)
    ...
    def feature_classification(self, cloud, curvatures, picked_list, scan_start_id, scan_end_id):
        ...
        for scan_id in range(self.used_line_num):
            ...
            for i in range(6):
                if scan_start_id[scan_id] is None:
                    continue
                else:
    ...
    def feature_extract(self, cloud):
        ...
        corner_sharp, corner_less, surf_flat, surf_less = self.feature_classification(cloud, curvatures, picked_list,
